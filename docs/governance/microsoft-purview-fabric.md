...
- **Microsoft Purview Data Loss Prevention (DLP)** - DLP policies are supported in Fabric for Power BI semantic models and for structured data in OneLake, such as lakehouses, SQL databases, Kusto databases and mirrored databases. DLP policies detect upload of sensitive data into these items. They can detect sensitivity labels and sensitive info types, such as credit card and social security numbers. They can be configured to generate policy tips and alerts for security admins. DLP policies can also be configured to allow workspace admins to override them. For more information, see [data loss prevention policies](/power-bi/enterprise/service-security-dlp-policies-for-power-bi-overview).
