...
| Preview experience shows no data | This usually indicates a data access issue. <br><br>One possible cause is having insufficient permissions to Fabric resources. Ensure that you have **Contributor** level access (not just **Viewer**) in both your Fabric workspace and the data source used for bindings in the ontology. <br><br>This error might also occur when your ontology instance can't access the underlying Fabric Graph. Ontology only supports **managed** lakehouse tables (located in the same OneLake directory as the lakehouse), not **external** tables that show in the lakehouse but reside in a different location. The preview experience connection might also be broken by changing the table name after mappings are created. |
