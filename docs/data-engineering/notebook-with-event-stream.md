...
1. **Discover and add eventstream into Notebook**:
    - Open your Microsoft Fabric notebook.
    - Select on **Add data items** under the **Data items** tab, then select **From Real-Time hub** from the dropdown menu.
    - In the Real-Time hub dialog, locate the target Eventstream you want to query with different search and filter options.
    - Add the Eventstream to your notebook environment.
...
2. **Read streaming data from eventstream**:
    - From the context menu of the Eventstream item in the object explorer, select **Read with Spark** to create a preconfigured code cell to read from the selected Eventstream.
    ...
    - In the newly created code cell, you can use the following PySpark code snippet to read streaming data from the Eventstream:
