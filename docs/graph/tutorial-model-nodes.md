...
In this tutorial step, you learn how to add node types to your graph model. Nodes represent entities in your data, such as customers, products, or orders. Later, you connect these nodes with edges to define relationships between them.

## Adventure Works node mappings

In the Adventure Works data model, create a node types for each entity. The following table shows the node mappings. Use this information to add nodes to your graph:
...
1. In your graph model, select **Add node** to add a new node types to your graph.
...
> [!TIP]
> You may set compound keys (IDs consisting of multiple columns).
...
1. Repeat the process for all other node types in the Adventure Works data model:
    ...
    | Vendor             | adventureworks_vendors              | VendorID_K           |
> [!TIP]
> When you double-click on a node type, you can see its properties. Each property maps to a column in the source table. Delete properties that you will not need in queries or analysis, because excessive properties make your graph harder to maintain and use.
...
Now that you added nodes to your graph, the next step is to add edges to define the relationships between these nodes.
    > [!TIP]
    > Besides creating node types from entire tables, any column (or set of columns) from any table can form a standalone node type if it represents an entity that you need on the graph. For example, you can create a **country** node type from the **country** column in the **adventureworks_employees** table, with **country**being the ID; delete properties that are not required for the uniqueness of the **country** nodes, such as employee name, employee ID, job title, gender, etc.
## Next step
