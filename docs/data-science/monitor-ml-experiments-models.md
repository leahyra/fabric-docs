---
title: Monitor ML experiments and models
description: Learn how to monitor machine learning experiments from the Fabric Monitoring hub and track traffic for ML model endpoints.
ms.author: ruxu
author: ruixinxu
ms.reviewer: 
reviewer: 
...
- At least one [machine learning experiment](machine-learning-experiment.md) with recorded runs, or an [ML model](machine-learning-model.md) with an active [ML model endpoint](model-endpoints.md).

## Monitor ML experiments from the Monitoring hub

ML experiments are integrated directly into the Fabric Monitoring hub. This integration provides a centralized view of all experiment activities, the related notebooks, Spark applications, and the ML experiment runs those applications generate. With the Monitoring hub, you can track, filter, and troubleshoot your experiment runs without navigating away from a single unified experience.

### View experiment runs in the Monitoring hub

To view ML experiment runs from the Monitoring hub:
...
These filters make it easier to manage and analyze experiments, especially in workspaces with a high volume of ML activities.

### Track related ML experiment runs from notebook activity

ML experiments are also integrated into the notebook activity view in the Monitoring hub. When you select a notebook activity which triggered a ML experiment run, you can access the **Item snapshots** page to see a snapshot of the experiments and runs captured at the time of execution. This page also displays a snapshot of all settings and parameters that were in effect when the notebook ran.
...
The ML model endpoint metric view provides key metrics about endpoint usage, including:
...
:::image type="content" source="media/monitor-ml-experiments-models/endpoint-metrics.png" alt-text="Screenshot showing endpoint metrics for an ML model." lightbox="media/monitor-ml-experiments-models/endpoint-metrics.png":::
...
> [!NOTE]
> ML models can have active endpoints for up to five versions at a time. Traffic monitoring is available for each active version endpoint independently.
...
For detailed information about endpoint capacity consumption and billing, see [ML model endpoint consumption rates](model-endpoints.md#consumption-rate).
...
- [Serve real-time predictions with ML model endpoints](model-endpoints.md)
