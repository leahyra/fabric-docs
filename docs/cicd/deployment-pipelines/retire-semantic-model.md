...
## What happens:
 ...
 - Attempting to deploy nonupgraded semantic models results in deployment failure.
...
- If a report has unapplied query changes or upgrade errors, users see a warning and must manually upgrade the report.
...
Semantic Models using Live Connections to external AS servers (aka ASLC models) are only available with nonenhanced metadata, and can't be upgraded to enhanced metadata. Microsoft Fabric deployment pipelines continues to support these ASLC Semantic Models  
