...
- **Filter**: Specifies selection filter using query operators. To return all documents in a collection, omit this parameter or pass an empty document ({}). For more information about querying Azure Cosmos DB for Mongo DB, see [these query tutorials](/azure/cosmos-db/mongodb/tutorial-query)
